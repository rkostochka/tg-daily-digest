...
приложенных нет или её не удалось загрузить — отвечай по дайджесту и честно
отметь, что полного текста под рукой не было.
...
async def deep_dive(
    ...
    """
    articles_block = await _build_articles_block(digest_text, source_urls)
    ...
    user_prompt = (
        ...
        f"в заданном формате. Если приложены полные тексты статей — опирайся на них. "
